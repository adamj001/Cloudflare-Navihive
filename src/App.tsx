...
function App() {
  ...
  // 在 fetchData 成功后自动选中第一个
  useEffect(() => {
    if (groups.length > 0 && !groups.find(g => g.id === selectedTab)) {
    ...
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        <Container maxWidth="xl" sx={{ py: 3, position: 'relative', zIndex: 2 }}>
          {/* 顶部固定 AppBar + Tabs 导航 */}
          <AppBar position="sticky" color="transparent" elevation={0} sx={{ mb: 4, backdropFilter: 'blur(10px)', bgcolor: 'background.paper' + 'ee' }}>
            ...
            <Box sx={{ mb: 4, maxWidth: 600, mx: 'auto' }}>
              <SearchBox
                ...
                onInternalResultClick={(result) => {
                  const el = document.getElementById(`group-${result.type === 'group' ? result.id : result.groupId}`);
                ...
                .map((group) => (
                  <Box key={group.id} id={`group-${group.id}`}>
                    <GroupCard
                      ...
                      onStartSiteSort={() => startSiteSort(group.id!)}
